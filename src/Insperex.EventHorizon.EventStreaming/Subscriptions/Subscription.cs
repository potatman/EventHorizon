...
public class Subscription<T> : IAsyncDisposable where T : class, ITopicMessage, new()
{
    ...
    private async Task<MessageContext<T>[]> LoadEvents()
    {
        ...
        try
        {
            ...
            if (batch?.Any() == true)
            {
                ...
                _logger.LogInformation("Subscription - Loaded {Type}(s) {Count} in {Duration} {@Config}",
    ...
    private async Task OnEvents(MessageContext<T>[] batch)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Subscription - Processed {Type}(s) {Count} in {Duration}, from {Start}-{End} {@Config}",
